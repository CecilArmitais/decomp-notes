# `0f08744f` — Decomp five team-member index accessors

| | |
|---|---|
| **Commit** | `0f08744f` (as of writing — renamed if amended or rebased) |
| **Branch** | `decomp-team-member-accessors`, on top of `951ccd61` |
...
The index-returning counterparts of the pointer accessors in `951ccd61`.
...
- **Fact.** The distinction is visible **only** for `GetHeroMemberIdx`, whose
  ...
  types requires it.
- **Fact.** `sub_02065050` (commit `7052c024`) calls `GetMainCharacter1MemberIdx`
