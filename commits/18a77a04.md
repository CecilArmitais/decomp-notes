# `18a77a04` — Decomp sub_02055410, sub_02055474 and sub_020554D8

| | |
|---|---|
| **Commit** | `18a77a04` (as of writing — renamed if amended or rebased) |
| **Branch** | `decomp-team-member-accessors`, on top of `1f97565e` |
...
1. Those are exactly the indices `GetMainCharacter1`, `GetMainCharacter2` and
   `GetMainCharacter3` select (commit `cb366895`), including `GetMainCharacter3`
...
- **`members[(s16)idx]`** with an `int` parameter — the cast gives `smulbb` for
  ...
  comparisons. Same lever as the `*MemberIdx` functions in `bee91dbc`.
