# `fc3680b1` — Rephrase eight matched functions; drop index casts and magic numbers

| | |
|---|---|
| **Commit** | `fc3680b1` (as of writing — renamed if amended or rebased) |
| **Branch** | `decomp-continued`, on top of [`5ebe15c4`](5ebe15c4.md) |
...
the commit** (`git show fc3680b1 | grep '^[+-]extern'` is empty). The only
...
If the note for [`5ebe15c4`](5ebe15c4.md) repeats that claim, **this commit
...
1. [`bd3c6e4a`](bd3c6e4a.md) recorded that `ov11_02307334`'s *eight function-scope
...
2. [`b1b5c94f`](b1b5c94f.md) — the local-renaming commit — was careful to say
   *"Declaration order is untouched … so no object changes."* Here declaration
   ...
   That is not a contradiction (b1b5c94f was avoiding a risk, not asserting the
...
none should be inferred — unlike [`bd3c6e4a`](bd3c6e4a.md), which does claim all
three.
